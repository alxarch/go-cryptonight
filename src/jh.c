...
static void E8(jhHashState *state);  /*The bijective function E8, in bitslice form*/
static void F8(jhHashState *state);  /*The compression function F8 */

/*The API functions*/
static jhHashReturn JhInit(jhHashState *state, int hashbitlen);
static jhHashReturn JhUpdate(jhHashState *state, const jhBitSequence *data, jhDataLength databitlen);
static jhHashReturn JhFinal(jhHashState *state, jhBitSequence *hashval);
...
static void E8(jhHashState *state)
{
...
static void F8(jhHashState *state)
{
      uint64  i;
...
static jhHashReturn JhUpdate(jhHashState *state, const jhBitSequence *data, jhDataLength databitlen)
{
      jhDataLength index; /*the starting address of the data to be compressed*/
...
jhHashReturn jh_hash(int hashbitlen, const jhBitSequence *data,jhDataLength databitlen, jhBitSequence *hashval)
{
      ...
      if ( hashbitlen == 224 || hashbitlen == 256 || hashbitlen == 384 || hashbitlen == 512 ) {
            JhInit(&state, hashbitlen);
            JhUpdate(&state, data, databitlen);
            JhFinal(&state, hashval);
            return SUCCESS;
      }
      else
            return(BAD_HASHLEN);
