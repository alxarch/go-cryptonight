...
static void Update(groestlHashState* ctx,
		  const groestlBitSequence* input,
		  groestlDataLength databitlen) {
  int index = 0;
...
static void Final(groestlHashState* ctx,
		 groestlBitSequence* output) {
  int i, j = 0, hashbytelen = HASH_BIT_LEN/8;
  uint8_t *s = (groestlBitSequence*)ctx->chaining;
...
/* hash bit sequence */
void groestl(const groestlBitSequence* data, 
		groestlDataLength databitlen,
		groestlBitSequence* hashval) {
