...
**   After an InitExt() call, just use SkeinUpdate/SkeinFinal calls as with SkeinInit().
**
**   Notes: Same parameters as _Init() calls, plus treeInfo/key/keyBytes.
**          When keyBytes == 0 and treeInfo == SKEIN_SEQUENTIAL, 
**              the results of InitExt() are identical to calling SkeinInit().
**          The function SkeinInit() may be called once to "precompute" the IV for
...
** avoid the need for a Threefish call during SkeinInit().
**
...
static int Skein_256_InitExt(Skein_256_Ctxt_t *ctx,size_t hashBitLen,u64b_t treeInfo, const u08b_t *key, size_t keyBytes)
    ...
    if (keyBytes == 0)                          /* is there a key? */
        ...
        /* do a mini-SkeinInit right here */
...
static skeinHashReturn SkeinUpdate(skeinHashState *state, const skeinBitSequence *data, skeinDataLength databitlen)
{
  /* only the final SkeinUpdate() call is allowed do partial bytes, else assert an error */
